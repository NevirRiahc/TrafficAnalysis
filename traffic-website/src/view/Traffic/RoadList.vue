<template>
  <div>
    <div id="JobChance">
      <div class="banner container-fuild text-center">TRAFFIC FLOW</div>
      <div class="container">
...
<!--              <th>状态</th>-->
            ...
            <tr v-for="(item,index) in tableDatas">
              ...
              <td>{{item.blockid}}</td>
              <td>{{item.gotime}}</td>
<!--              <td><span class="badge badge-info">{{item.exponent}}</span></td>-->
              <td>{{item.flow}}</td>
              <td>{{item.time1}}</td>
            </tr>
            </tbody>
...
<script>
  ...
  import {list as listRoad} from "@/api/road";
